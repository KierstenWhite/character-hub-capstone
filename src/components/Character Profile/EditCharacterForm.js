//Handles data and JSX for editing the character form

import "./CharacterForm.css"
import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import {
  GetIndividualCharacterProfile,
  GetProfileAndReplace,
} from "./CharacterAPI";
import {
  Button,
  Divider,
...
export const EditCharacterForm = () => {
  ...
  useEffect(() => {
    GetIndividualCharacterProfile(characterId).then((data) => {
      editCharacter(data);
    });
  }, [characterId]);
  ...
  const handleSaveButtonClick = (event) => {
    ...
    GetProfileAndReplace(character).then(() => {
  ...
  return (
    <>
      <Form id="editCharacterForm">
        <Header as="h2" id="formTextHeader">Edit Character Form</Header>
